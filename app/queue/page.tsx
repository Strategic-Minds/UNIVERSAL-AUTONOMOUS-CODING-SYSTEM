...
const ITEMS = [
  ...
  {id:5,p:"P3",name:"NexusReach v2",desc:"Apollo-inspired sales platform",phase:"IDEA",agent:"APEX",status:"queued",pct:0,queued:"Jul 27",eta:"Aug 1"},
  ...
  {id:8,p:"P4",name:"70-City Epoxy Network",desc:"SEO landing pages",phase:"IDEA",agent:"APEX",status:"queued",pct:0,queued:"Jul 27",eta:"Aug 10"},
]
const TABS = ["All","Idea","In Progress","Awaiting Approval","Testing","Queued","Completed"]
const PHASE_DOTS = ["IDEA","SPEC","BRAND","BUILD","VALIDATE","DEPLOY"]
const PHASE_IDX:Record<string,number>={IDEA:0,SPEC:1,BRAND:2,BUILD:3,VALIDATE:4,DEPLOY:5}

export default function Queue() {
  const [tab,setTab]=useState("All")
  const items=tab==="All"?ITEMS:ITEMS.filter(i=>i.status.includes(tab.toLowerCase().replace(" ","_"))||i.phase.toLowerCase()===tab.toLowerCase())
      ...
      <div className="page-wrapper">
        <div className="page-header">
          <div>
            ...
            <p className="page-subtitle">{ITEMS.length} items · {ITEMS.filter(i=>i.status==="in_progress").length} active</p>
          </div>
                    ...
                    <div style={{display:"flex",gap:4,alignItems:"center",marginBottom:10}}>
                      {PHASE_DOTS.map((ph,i)=>(
                        <div key={ph} style={{display:"flex",alignItems:"center",gap:4}}>
                          <div style={{width:i===phaseIdx?24:8,height:8,borderRadius:20,background:i<phaseIdx?"#22c55e":i===phaseIdx?"#f8b800":"#e5e7eb",transition:"all .2s"}}/>
                          {i<PHASE_DOTS.length-1 && <div style={{width:8,height:1,background:"#e5e7eb"}}/>}
                        </div>
                      ))}
